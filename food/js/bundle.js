...
function calc() {
  ...
  var ratio, sex, weight, age, height;
...
function forms(formSelector, modalTimerId) {
  ...
  function bindPostData(form) {
    form.addEventListener('submit', function (event) {
      ...
      form.insertAdjacentElement('afterend', statusMessage); // const req = new XMLHttpRequest();
      ...
      var formData = new FormData(form); //1 перевод из formData в json

      var json = JSON.stringify(Object.fromEntries(formData.entries())); //2 перевод из formData в json
...
function slider(_ref) {
  ...
  if (slides.length < 10) {
    ...
    current.textContent = "0".concat(indexSlide);
  } else {
  ...
  next.addEventListener('click', function () {
    ...
    slidesInner.style.transform = "translateX(-".concat(offset, "px)");
